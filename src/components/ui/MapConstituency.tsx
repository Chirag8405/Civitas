...
export function MapConstituency({
	...
	const drawingRef = React.useRef<google.maps.drawing.DrawingManager | null>(
		null
	);
	const boundaryRef = React.useRef<google.maps.Polygon | null>(null);
		const mapRef = React.useRef<null | any>(null);
		const drawingRef = React.useRef<null | any>(null);
		const boundaryRef = React.useRef<null | any>(null);
		const boothMarkersRef = React.useRef<any[]>([]);

			Array<{ polygon: any; label: any }>
