...
import { ELECTION_ID, TOTAL_VOTERS } from "@/lib/constants";
...
import type { VoteRecord, Dispute } from "@/types";
import { DisputeModal } from "@/components/ui/DisputeModal";
import { ErrorBoundary } from "@/components/ErrorBoundary";

export default function PollingPage() {
  const router = useRouter();
  const { constituency, results, updateResults } = useSimulationStore();
      ...
      snapshot.docChanges().forEach((change) => {
        if (change.type === "added") {
          ...
          if (data.status === "PENDING") {
            setPendingDispute({ id: change.doc.id, ...data } as Dispute);
          }
        }
      });
    });
  ...
  const handleRuling = async (resolution: "ACCEPT" | "REJECT") => {
    ...
    try {
      ...
      console.error("[polling] Dispute update failed:", message);
    }
    ...
    if (resolution === "REJECT") {
      ...
      adjustedVotes = adjustedVotes.filter((v) => {
        if (v.zoneId === dispute.zoneId && removed < dispute.votesAffected) {
          removed++;
          return false;
        }
        return true;
  ...
  const candidateCounts = useMemo(() => {
    const electionStore = useSimulationStore.getState().election;
    const counts = electionStore.candidates.map((c) => ({
      ...c,
  ...
  const chartData = useMemo(() => {
    return candidateCounts.map((c) => ({
      name: c.name,
      votes: c.votes,
      fill: c.id === leadingCandidateId ? "#C0392B" : "#1A1A2E",
    }));
